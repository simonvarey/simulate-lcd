...
fn neighbour_count<const C: usize, const R: usize>(bm: &Bitmap<C, R>, nrow: usize, ncol: usize) -> usize {
...
fn next_state<const C: usize, const R: usize>(old: Bitmap<C, R>) -> Bitmap<C, R> {
  ...
  for nrow in 0..R {
    for ncol in 0..C {
      ...
      if old[nrow][ncol] {
        if n_neighbours == 2 || n_neighbours == 3 { new[nrow][ncol] = true }
      } else {
        if n_neighbours == 3 { new[nrow][ncol] = true }
      }
    }
  }
  new
...
fn main() {
  ...
  let mut screen = LcdScreen::<65, 120>::new(&sdl_context, "Game of Life", 
